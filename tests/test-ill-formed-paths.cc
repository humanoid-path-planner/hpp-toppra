...
TEST_CASE("Path of length 0") {
  ...
  hc::PathVectorPtr_t pv =
      hc::PathVector::create(device->configSize(), device->numberDof());
  hp::vector_t q1(device->configSize()), q2(device->configSize()),
  ...
  if (!addPathBefore && !addPathAfter) CHECK(res->length() == 0.);
