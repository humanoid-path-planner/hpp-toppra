...
void print_path_evaluation(hc::PathPtr_t p, int N) {
  hp::vector_t q(p->outputSize()), v(p->outputDerivativeSize()),
      a(p->outputDerivativeSize());
  hp::value_type t = 0.0, t0 = p->timeRange().first, t1 = p->timeRange().second;
  std::cout << "Time interval: " << t0 << ", " << t1 << "\nt, q, v, a\n";
  for (int i = 0; i <= N; ++i) {
...
hp::DevicePtr_t makeDevice() {
  ...
  hp::urdf::loadModelFromString(
      device, 0, "", "prismatic_x",
...
hc::PathVectorPtr_t makeQuinticSpline(hc::ProblemPtr_t p, hc::value_type v0,
                                      hc::value_type v1) {
  auto sm = SteerSplineOrder5::create(p);
  hp::vector_t q1(p->robot()->neutralConfiguration()), q2(q1);
  hp::matrix_t d1(p->robot()->numberDof(), 2), d2(p->robot()->numberDof(), 2);
...
TEST_CASE("Test throw on invalid parameters") {
  ...
  std::string gridpointMethod =
      GENERATE("time_space", "param_space", "throw: invalid");
  std::string interpolationMethod =
      GENERATE("hermite", "constant_acceleration", "throw: invalid");
  bool expect_throw = (interpolationMethod.compare(0, 5, "throw") == 0 ||
                       gridpointMethod.compare(0, 5, "throw") == 0);

  CAPTURE(gridpointMethod, expect_throw);

  auto problem = hc::Problem::create(device);
  problem->setParameter("PathOptimization/TOPPRA/gridpointMethod",
                        hc::Parameter(gridpointMethod));
  problem->setParameter("PathOptimization/TOPPRA/interpolationMethod",
                        hc::Parameter(interpolationMethod));
...
TEST_CASE("Test parameters") {
  ...
  problem->setParameter("PathOptimization/TOPPRA/N",
                        hc::Parameter(hp::size_type(200)));
  problem->setParameter("PathOptimization/TOPPRA/interpolationMethod",
...
#define PARAM(name, type)                                                    \
  INFO(#name ": " << problem->getParameter("PathOptimization/TOPPRA/" #name) \
                         .type##Value());
...
TEST_CASE("Run TOPPRA optimizer: all cases") {
  ...
  std::string interpolationMethod =
      GENERATE("hermite", "constant_acceleration");
  ...
  hp::vector_t q(device->configSize()), v(device->numberDof()),
      a(device->numberDof());
  ...
  for (int i = 0; i <= N; ++i) {
    ...
    hc::value_type s = param->value(t), sd = param->derivative(t, 1),
...
TEST_CASE("PiecewisePolynomial") {
  ...
  constexpr int N = 4;
  typedef hc::timeParameterization::ShiftedPiecewisePolynomial<order>
      TimeParam_t;
  auto c = TimeParam_t::ParameterMatrix_t(order + 1, N);
  c << 0.0, 1.0, 2.0, 3.0, 0.5, 0.5, 0.5, 0.5, 0.5, -0.5, 0.5, -0.5;
  hp::vector_t t(N + 1);
  ...
  CHECK_THROWS(param.value(
      t[0] - 2 * Eigen::NumTraits<hc::value_type>::dummy_precision()));
}
